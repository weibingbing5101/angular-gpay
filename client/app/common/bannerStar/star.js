...
function init(sceneNode) {
	sceneNode= sceneNode||document.body;

	let sceneWidth= sceneNode.clientWidth,
			sceneHeight = sceneNode.clientHeight;
	// 
	camera = new THREE.PerspectiveCamera(
			100, // 数值越大，星星出现的初始位置的z轴越远
			sceneWidth / sceneHeight, // 宽高比
			1, //能看到的最近距离
			4000 // 能看到的最远距离
		);
	...
	starTimer = setInterval(update, 1000 / 30);
...
function setParticleFadePos(id){
	if(!particleFadePos[id]){
		...
		runByRandom([20, 30, 40, 10], function(index){
