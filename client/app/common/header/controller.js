class Controller {
    constructor($rootScope, $cookies, $scope, $location, pathConfigFactory, homeSvc) {
        'ngInject';
        ...
        */
        ...
        this.logoutSelected= this.$location.path();
    };
    ...
    gotoMine() {
        let preUrl = this.$location.url();        

        if(preUrl==='/mine/total'){
            setTimeout(() => {
    ...
    changeSelectedPath(path){
        this.logoutSelected = path;     
        if (path=='/doc/detail/mpay' || path=='/doc/detail/net' || path=='/doc/detail/index') {
            this.logoutSelected = '/doc/guide';
        }
        if (path=='/help/refund' || path=='/help/fee') {
            this.logoutSelected = '/help/account';
        }   
    }
    ...
    addListener() {
        ...
        // $locaexitFNtionChangeStart hash变化后 页面变化前 给需要的页面添加APPID 和 APPNAME
        this.$rootScope.$on('$locationChangeStart', function(event, toState, fromUrl) {
            ...
            if (['/home', '/', ''].indexOf(path) > -1) {
             ...
             if(this.$cookies.get('token')&&this.$cookies.get('email'))
            {
